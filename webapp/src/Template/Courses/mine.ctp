...
<div id="marketing" class="container">
    <?php foreach($dlince_courses as $course) {
      ...
      echo "\n<!-- <CURSO -->";
      if ( ($this->request->session()->read('Auth.User'))!==null && ($this->request->session()->read('Auth.User.role.name'))!==null && ($this->request->session()->read('Auth.User.role.name')=='student' || $this->request->session()->read('Auth.User.role.name')=='teacher') )
      {
